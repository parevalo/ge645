...
def io_uncol_down(deltaL, nlayers, gdir, io, mp):
    ...
    for k in range(nlayers):
        L2 = (k+1) * deltaL - (0.5 * deltaL) # eric has i+1
    ...
    for i in range(ng/2, ng):
        for j in range(ng):
            ...
            for k in range(nlayers-1, -1, -1):
                L1 = (k+1) * deltaL - (0.5 * deltaL)  # Eric has k+1
...
def id_uncol_down(deltaL, nlayers, gdif, i_d, ng, xg):
    ...
    for i in range(ng/2):
        for j in range(ng):
            L1 = 0.0
            for k in range(nlayers):
                L2 = (k+1) * deltaL - (0.5 * deltaL)  # Eric has k+1
                prob = np.exp(-(1 / np.abs(xg[i])) * gdif[j, i] * (L2 - L1))
...
def sweep_up(nlayers, ng, xg, gdif, deltaL, jj, ic, eps):
    ...
    for i in range(ng/2, ng):
        for j in range(ng):
            ...
            dij = 1.0 / fij
            for k in range(nlayers-1, -1, -1):
                ic[k, j, i] = cij * ic[k, j, i] + dij * jj[k, j, i]
...
def energy_bal(nlayers, ng, xg, wg, deltaL, r_s, rhold, tauld, gdir, gdif, fo_ucdsoil,
    ...
    for i in range(ng/2):
        sum2 = 0.0
        for j in range(ng):
            sum2 += wg[j] * ic[nlayers, j, i] #nlay+1 in original
    ...
    for i in range(ng/2, ng):
        ...
        for j in range(ng):
            sum2 += wg[j] * ic[1, j, i]
            #changed to 1 to match ic above
            #it improves the balance a bit
    ...
    #  Evaluate canopy absorption from Id_udu
    sum1 = 0.0
    for k in range(nlayers):
        for i in range(ng/2, ng):
            sum2 = 0.0
